...
def build_day_actions_map(user, day_orders, menu_items_by_day):
    ...
    for target_date, day_items in menu_items_by_day.items():
        ...
        for item in day_items or []:
            can_order_item, _ = can_order_for_menuitem_date(user, item, target_date)
            if can_order_item:
                orderable_items.append(item)

        actions[target_date] = {
            "mode": "order" if orderable_items else "none",
            "has_order": False,
            "date": target_date,
            "label": target_date.strftime("%d.%m.%Y"),
            "order_reason": "",
            "total_items": len(orderable_items),
            "items": [
                {
                    "name": item.jidlo.nazev,
                    "meal_type": item.druh_jidla.nazev if item.druh_jidla else "",
                }
...
def dashboard(request):
    ...
    my_day_orders = get_user_day_orders(request.user)
    calendar_order_badges = {
        entry["date"]: {
            "qty": entry.get("total_qty", 0),
            "items": len(entry.get("items", [])),
        }
        for entry in my_day_orders
    }
    calendar_order_days = set(calendar_order_badges.keys())
    ...
    context = {
        **calendar_ctx,
        ...
        'my_day_orders': my_day_orders,
        'calendar_order_badges': calendar_order_badges,
        'calendar_order_days': calendar_order_days,
    ...
    if create_day:
        ...
        for item in day_items:
            ...
            if not can_order_item:
                skipped_items.append(item.jidlo.nazev)
                continue
        ...
        if not items_to_create:
            return JsonResponse(
                {
                    'status': 'warning',
                    'message': 'Pro tento den není žádné další objednatelné jídlo.',
